...
typedef struct {
   ...
   /**
      @brief Flag to enable MMA computation
      0 => run FIR on C7x
      1 => run FIR on MMA
     */
   uint32_t enableMMA;
   ...
   /**
      @brief Size of MMA panels */

   int32_t MMA_SIZE;

   /**
      @brief Flag to enable float to Q format fixed point quantization computation on fixed point MMA only. Should only
      be enabled when input data types is floating point for a fixed point MMA implementation
      0 => run default
      1 => run Q fixed point on MMA
     */
   uint32_t enableQ;
