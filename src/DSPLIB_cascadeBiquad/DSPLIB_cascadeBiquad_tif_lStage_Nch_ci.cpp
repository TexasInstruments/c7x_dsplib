...
DSPLIB_STATUS DSPLIB_cascadeBiquad_tif_lStage_Nch_init_ci(DSPLIB_kernelHandle                  handle,
                                                          ...
                                                          const DSPLIB_cascadeBiquad_InitArgs *pKerInitArgs)
{
   ...
   se0Params.ICNT1         = DSPLIB_NUM_TIF_BIQUAD_COEFS;
   se0Params.DIM1          = (int32_t) eleCount * numStages * numChannelGroups;
   se0Params.ICNT2         = numStages;
   se0Params.DIM2          = (int32_t) eleCount * numChannelGroups;
   se0Params.ICNT3         = numChannelGroups;
   se0Params.DIM3          = (int32_t) eleCount;
...
DSPLIB_cascadeBiquad_tif_lStage_Nch_init_ci<float>(DSPLIB_kernelHandle                  handle,
                                                   const DSPLIB_bufParams2D_t          *bufParamsIn,
                                                   const DSPLIB_bufParams2D_t          *bufParamsFilterCoeff,
                                                   const DSPLIB_bufParams2D_t          *bufParamsFilterVar,
                                                   const DSPLIB_bufParams2D_t          *bufParamsOut,
                                                          ...
                                                          void *restrict pOut)
{
   ...
   pGainBuff         = (dataType *) pFilterCoeff + numStages * DSPLIB_NUM_TIF_BIQUAD_COEFS * numChannelsVecLen;
...
#pragma MUST_ITERATE(1, , )
   for (uint32_t chCount = 0; chCount < numChannels; chCount += eleCount) {
      ...
      // Stage 1
      b11_a11 = c7x::strm_eng<0, vec>::get_adv();
      b12_a12 = c7x::strm_eng<0, vec>::get_adv();
      ...
      // Stage 1
      __vpred filtVarsRd    = c7x::strm_agen<2, vec>::get_vpred();
      vec    *filtVarsRdPtr = c7x::strm_agen<2, vec>::get_adv(pFilterVarLocal);
      vec     filtVars_d0_1 = __vload_pred(filtVarsRd, filtVarsRdPtr);
      filtVarsRd            = c7x::strm_agen<2, vec>::get_vpred();
      filtVarsRdPtr         = c7x::strm_agen<2, vec>::get_adv(pFilterVarLocal);
      vec filtVars_d1_1     = __vload_pred(filtVarsRd, filtVarsRdPtr);
...
#pragma MUST_ITERATE(1, , )
      ...
      // Stage 1
      __vpred filtVarsWr    = c7x::strm_agen<1, vec>::get_vpred();
      vec    *filtVarsWrPtr = c7x::strm_agen<1, vec>::get_adv(pFilterVarLocal);
      __vstore_pred(filtVarsWr, filtVarsWrPtr, filtVars_d0_1);
      filtVarsWr    = c7x::strm_agen<1, vec>::get_vpred();
      filtVarsWrPtr = c7x::strm_agen<1, vec>::get_adv(pFilterVarLocal);
      __vstore_pred(filtVarsWr, filtVarsWrPtr, filtVars_d1_1);
   }
...
template DSPLIB_STATUS DSPLIB_cascadeBiquad_tif_1Stage_Nch_exec_ci<float>(DSPLIB_kernelHandle handle,
                                                                          void *restrict pIn,
                                                                          void *restrict pFilterCoeff,
                                                                          void *restrict pFilterVar,
                                                                          void *restrict pOut);
...
#pragma MUST_ITERATE(1, , )
   for (uint32_t chCount = 0; chCount < numChannels; chCount += eleCount) {
      ...
      // Stage 2
      b21_a21 = c7x::strm_eng<0, vec>::get_adv();
      b22_a22 = c7x::strm_eng<0, vec>::get_adv();
      ...
      filtVarsRd        = c7x::strm_agen<2, vec>::get_vpred();
      filtVarsRdPtr     = c7x::strm_agen<2, vec>::get_adv(pFilterVarLocal);
      vec filtVars_d0_2 = __vload_pred(filtVarsRd, filtVarsRdPtr);
      filtVarsRd        = c7x::strm_agen<2, vec>::get_vpred();
      filtVarsRdPtr     = c7x::strm_agen<2, vec>::get_adv(pFilterVarLocal);
      vec filtVars_d1_2 = __vload_pred(filtVarsRd, filtVarsRdPtr);
...
#pragma MUST_ITERATE(1, , )
      ...
      filtVarsWr    = c7x::strm_agen<1, vec>::get_vpred();
      filtVarsWrPtr = c7x::strm_agen<1, vec>::get_adv(pFilterVarLocal);
      __vstore_pred(filtVarsWr, filtVarsWrPtr, filtVars_d0_2);
      filtVarsWr    = c7x::strm_agen<1, vec>::get_vpred();
      filtVarsWrPtr = c7x::strm_agen<1, vec>::get_adv(pFilterVarLocal);
      __vstore_pred(filtVarsWr, filtVarsWrPtr, filtVars_d1_2);
   }
...
template DSPLIB_STATUS DSPLIB_cascadeBiquad_tif_2Stage_Nch_exec_ci<float>(DSPLIB_kernelHandle handle,
                                                                          void *restrict pIn,
                                                                          void *restrict pFilterCoeff,
                                                                          void *restrict pFilterVar,
                                                                          void *restrict pOut);
...
template <typename dataType>
DSPLIB_STATUS DSPLIB_cascadeBiquad_tif_3Stage_Nch_exec_ci(DSPLIB_kernelHandle handle,
                                                          void *restrict pIn,
                                                          void *restrict pFilterCoeff,
                                                          void *restrict pFilterVar,
                                                          void *restrict pOut)
...
#pragma MUST_ITERATE(1, , )
   for (uint32_t chCount = 0; chCount < numChannels; chCount += eleCount) {
      ...
      // Stage 3
      b31_a31 = c7x::strm_eng<0, vec>::get_adv();
      b32_a32 = c7x::strm_eng<0, vec>::get_adv();
      ...
      filtVarsRd        = c7x::strm_agen<2, vec>::get_vpred();
      filtVarsRdPtr     = c7x::strm_agen<2, vec>::get_adv(pFilterVarLocal);
      vec filtVars_d0_3 = __vload_pred(filtVarsRd, filtVarsRdPtr);
      filtVarsRd        = c7x::strm_agen<2, vec>::get_vpred();
      filtVarsRdPtr     = c7x::strm_agen<2, vec>::get_adv(pFilterVarLocal);
      vec filtVars_d1_3 = __vload_pred(filtVarsRd, filtVarsRdPtr);
...
#pragma MUST_ITERATE(1, , )
      ...
      // Stage 1
      __vpred filtVarsWr     = c7x::strm_agen<1, vec>::get_vpred();
      vec    *filterVarsWPtr = c7x::strm_agen<1, vec>::get_adv(pFilterVarLocal);
      __vstore_pred(filtVarsWr, filterVarsWPtr, filtVars_d0_1);
      filtVarsWr     = c7x::strm_agen<1, vec>::get_vpred();
      filterVarsWPtr = c7x::strm_agen<1, vec>::get_adv(pFilterVarLocal);
      __vstore_pred(filtVarsWr, filterVarsWPtr, filtVars_d1_1);

      // Stage 2
      filtVarsWr     = c7x::strm_agen<1, vec>::get_vpred();
      filterVarsWPtr = c7x::strm_agen<1, vec>::get_adv(pFilterVarLocal);
      __vstore_pred(filtVarsWr, filterVarsWPtr, filtVars_d0_2);
      filtVarsWr     = c7x::strm_agen<1, vec>::get_vpred();
      filterVarsWPtr = c7x::strm_agen<1, vec>::get_adv(pFilterVarLocal);
      __vstore_pred(filtVarsWr, filterVarsWPtr, filtVars_d1_2);

      // Stage 3
      filtVarsWr     = c7x::strm_agen<1, vec>::get_vpred();
      filterVarsWPtr = c7x::strm_agen<1, vec>::get_adv(pFilterVarLocal);
      __vstore_pred(filtVarsWr, filterVarsWPtr, filtVars_d0_3);
      filtVarsWr     = c7x::strm_agen<1, vec>::get_vpred();
      filterVarsWPtr = c7x::strm_agen<1, vec>::get_adv(pFilterVarLocal);
      __vstore_pred(filtVarsWr, filterVarsWPtr, filtVars_d1_3);
   }
...
template DSPLIB_STATUS DSPLIB_cascadeBiquad_tif_3Stage_Nch_exec_ci<float>(DSPLIB_kernelHandle handle,
                                                                          void *restrict pIn,
                                                                          void *restrict pFilterCoeff,
                                                                          void *restrict pFilterVar,
                                                                          void *restrict pOut);
