...
DSPLIB_STATUS DSPLIB_cascadeBiquad_df1_init_ci(DSPLIB_kernelHandle                  handle,
                                               ...
                                               const DSPLIB_cascadeBiquad_InitArgs *pKerInitArgs)
{
   ...
   uint32_t numStageGroups = numStages / DSPLIB_CASCADEBIQUAD_DF1_STAGES_PER_STAGE_GROUP;
   // Calculate number of remaining stages
   uint32_t remStages = numStages % DSPLIB_CASCADEBIQUAD_DF1_STAGES_PER_STAGE_GROUP;

   // Store number of stages groups and number of remaining stages
   pKerPrivArgs->numStageGroups = numStageGroups;
   pKerPrivArgs->stage1Cnt      = remStages;
   ...
   sa0Params.ICNT2         = numStageGroups + remStages;
   sa0Params.DIM2          = 0;
   ...
   sa1Params.ICNT1         = numStages * DSPLIB_NUM_STATE_VARS_DF1;
   ...
   sa2Params.ICNT1         = numStages * DSPLIB_NUM_STATE_VARS_DF1;
...
template DSPLIB_STATUS DSPLIB_cascadeBiquad_df1_init_ci<float>(DSPLIB_kernelHandle         handle,
                                                               const DSPLIB_bufParams2D_t *bufParamsIn,
                                                               const DSPLIB_bufParams2D_t *bufParamsFilterCoeff,
                                                               const DSPLIB_bufParams2D_t *bufParamsFilterVar,
                                                               const DSPLIB_bufParams2D_t *bufParamsOut,
...
template <typename dataType>
DSPLIB_STATUS DSPLIB_cascadeBiquad_df1_exec_ci(DSPLIB_kernelHandle handle,
                                               void *restrict pIn,
                                               ...
                                               void *restrict pOut)
{
   ...
   uint32_t remStages      = pKerPrivArgs->stage1Cnt;
...
#pragma MUST_ITERATE(1, , )
   for (uint32_t chCount = 0; chCount < numChannels; chCount += eleCount) {
      ...
      for (uint32_t stageGroupCnt = 0; stageGroupCnt < numStageGroups; stageGroupCnt++) {
         ...
         vec     xd1_1           = __vload_pred(filterVarsRd, filterVarsRdPtr);
         ...
         vec xd2_1               = __vload_pred(filterVarsRd, filterVarsRdPtr);
         filterVarsRd            = c7x::strm_agen<2, c7x::float_vec>::get_vpred();
         filterVarsRdPtr         = c7x::strm_agen<2, c7x::float_vec>::get_adv(pFilterVarLocal);
         vec yd1_1               = __vload_pred(filterVarsRd, filterVarsRdPtr);
         filterVarsRd            = c7x::strm_agen<2, c7x::float_vec>::get_vpred();
         filterVarsRdPtr         = c7x::strm_agen<2, c7x::float_vec>::get_adv(pFilterVarLocal);
         vec yd2_1               = __vload_pred(filterVarsRd, filterVarsRdPtr);

         // Stage 2
         filterVarsRd    = c7x::strm_agen<2, c7x::float_vec>::get_vpred();
         filterVarsRdPtr = c7x::strm_agen<2, c7x::float_vec>::get_adv(pFilterVarLocal);
         vec xd1_2       = __vload_pred(filterVarsRd, filterVarsRdPtr);
         filterVarsRd    = c7x::strm_agen<2, c7x::float_vec>::get_vpred();
         filterVarsRdPtr = c7x::strm_agen<2, c7x::float_vec>::get_adv(pFilterVarLocal);
         vec xd2_2       = __vload_pred(filterVarsRd, filterVarsRdPtr);
         filterVarsRd    = c7x::strm_agen<2, c7x::float_vec>::get_vpred();
         filterVarsRdPtr = c7x::strm_agen<2, c7x::float_vec>::get_adv(pFilterVarLocal);
         vec yd1_2       = __vload_pred(filterVarsRd, filterVarsRdPtr);
         filterVarsRd    = c7x::strm_agen<2, c7x::float_vec>::get_vpred();
         filterVarsRdPtr = c7x::strm_agen<2, c7x::float_vec>::get_adv(pFilterVarLocal);
         vec yd2_2       = __vload_pred(filterVarsRd, filterVarsRdPtr);
...
#pragma MUST_ITERATE(1, , )
         for (uint32_t samp = 0; samp < dataSize; samp++) {
            ...
            // Calculate stage 1
            output_1 = (input * b0_1) + (xd1_1 * b1_1) + (xd2_1 * b2_1) - (yd1_1 * a1_1) - (yd2_1 * a2_1);
            xd2_1    = xd1_1;
            xd1_1    = input;
            yd2_1    = yd1_1;
            yd1_1    = output_1;
            ...
            // Calculate stage 2
            output_2 = (output_1 * b0_2) + (xd1_2 * b1_2) + (xd2_2 * b2_2) - (yd1_2 * a1_2) - (yd2_2 * a2_2);
            xd2_2    = xd1_2;
            xd1_2    = output_1;
            yd2_2    = yd1_2;
            yd1_2    = output_2;
         ...
         __vstore_pred(filterVarsWr, filterVarsWrPtr, xd1_1);
         filterVarsWr    = c7x::strm_agen<1, vec>::get_vpred();
         filterVarsWrPtr = c7x::strm_agen<1, vec>::get_adv(pFilterVarLocal);
         __vstore_pred(filterVarsWr, filterVarsWrPtr, xd2_1);
         filterVarsWr    = c7x::strm_agen<1, vec>::get_vpred();
         filterVarsWrPtr = c7x::strm_agen<1, vec>::get_adv(pFilterVarLocal);
         __vstore_pred(filterVarsWr, filterVarsWrPtr, yd1_1);
         filterVarsWr    = c7x::strm_agen<1, vec>::get_vpred();
         filterVarsWrPtr = c7x::strm_agen<1, vec>::get_adv(pFilterVarLocal);
         __vstore_pred(filterVarsWr, filterVarsWrPtr, yd2_1);

         // Stage 2
         filterVarsWr    = c7x::strm_agen<1, vec>::get_vpred();
         filterVarsWrPtr = c7x::strm_agen<1, vec>::get_adv(pFilterVarLocal);
         __vstore_pred(filterVarsWr, filterVarsWrPtr, xd1_2);
         filterVarsWr    = c7x::strm_agen<1, vec>::get_vpred();
         filterVarsWrPtr = c7x::strm_agen<1, vec>::get_adv(pFilterVarLocal);
         __vstore_pred(filterVarsWr, filterVarsWrPtr, xd2_2);
         filterVarsWr    = c7x::strm_agen<1, vec>::get_vpred();
         filterVarsWrPtr = c7x::strm_agen<1, vec>::get_adv(pFilterVarLocal);
         __vstore_pred(filterVarsWr, filterVarsWrPtr, yd1_2);
         filterVarsWr    = c7x::strm_agen<1, vec>::get_vpred();
         filterVarsWrPtr = c7x::strm_agen<1, vec>::get_adv(pFilterVarLocal);
         __vstore_pred(filterVarsWr, filterVarsWrPtr, yd2_2);
      }
...
#pragma MUST_ITERATE(1, , )
      ...
      // Reset local input buffer pointer for next channel group
...
template DSPLIB_STATUS DSPLIB_cascadeBiquad_df1_exec_ci<float>(DSPLIB_kernelHandle handle,
                                                               void *restrict pIn,
                                                               void *restrict pFilterCoeff,
                                                               void *restrict pFilterVar,
                                                               void *restrict pOut);
