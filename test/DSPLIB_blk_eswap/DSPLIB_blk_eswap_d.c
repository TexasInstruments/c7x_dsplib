      ...
      for (repCount = 0; repCount < numReps; repCount++) {
         ...
         if (pIn && pOut && pOutCn) {
            ...
            if (currPrm.dataTypeIn == DSPLIB_FLOAT32 || currPrm.dataTypeIn == DSPLIB_FLOAT64) {
               TI_fillBuffer_float_bounded(prm[tpi].testPattern, 0, pIn, prm[tpi].staticIn, bufParamsIn.dim_x, 1, 0,
            ...
            /* Always use byte-level comparison for blk_eswap because
             * endian-swapped float data produces NaN bit patterns that
             * TI_compare_mem_2D_float rejects even when both outputs match. */
