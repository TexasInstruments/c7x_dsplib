...
def gen_testCaseParams(testParamsFile, currPrm):
    ...
    idatFile.write(
        "DSPLIB_CALC_STRIDE(%s * sizeof(unsigned short), DSPLIB_ALIGN_SHIFT_64BYTES), // stride of P matrix \n"
        % ((strideP))
    )
...
def gen_test_case_header_file(testCase):
    ...
    if dType[0] == "f":
        in0 = np.random.uniform(low=minVal, high=maxVal, size=(order, order)).astype(
            "float32"
        )
