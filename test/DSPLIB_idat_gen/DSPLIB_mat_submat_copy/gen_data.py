...
def gen_testCaseParams(testParamsFile, currPrm):
    ...
    idatFile.write(
        "DSPLIB_CALC_STRIDE(%s * sizeof(%s), DSPLIB_ALIGN_SHIFT_64BYTES), // stride of X matrix \n"
        % ((strideX), (dType))
    )
    ...
    idatFile.write(
        "DSPLIB_CALC_STRIDE(%s * sizeof(%s), DSPLIB_ALIGN_SHIFT_64BYTES), // stride of Y matrix \n"
        % ((strideY), (dType))
    )
...
def gen_test_case_header_file(testCase):
    ...
    # print(dType)
    X = np.random.uniform(low=minVal, high=maxVal, size=(heightIn, widthIn)).astype(
        dType
    )
