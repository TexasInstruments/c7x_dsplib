...
def gen_testCaseParams(testParamsFile, currPrm):
    ...
    idatFile.write(
        "DSPLIB_CALC_STRIDE(%s * sizeof(%s), DSPLIB_ALIGN_SHIFT_64BYTES), // stride of B matrix \n"
        % ((strideIn1), (dType))
    )
    idatFile.write(
        "DSPLIB_CALC_STRIDE(%s * sizeof(%s), DSPLIB_ALIGN_SHIFT_64BYTES), // stride of C matrix \n"
        % ((strideOut), (dType))
    )
...
def gen_test_case_header_file(testCase):
    ...
    if dType == "int8_t":
        ...
        in0 = np.random.randint(
            low=minVal, high=maxVal, size=(M, K)
        )  # .astype("int16")
        in1 = np.random.randint(
            low=minVal, high=maxVal, size=(K, N)
        )  # .astype("int16")
