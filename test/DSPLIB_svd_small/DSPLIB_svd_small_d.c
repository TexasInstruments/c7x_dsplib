...
#include "DSPLIB_svd_small_check.h"
...
int DSPLIB_svd_small_d(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   ...
   DSPLIB_svd_small_testParams_t *prm;
   DSPLIB_svd_small_testParams_t  currPrm;
   DSPLIB_svd_small_getTestParams(&prm, &test_cases);

   DSPLIB_svd_small_InitArgs kerInitArgs;

   int32_t             handleSize = DSPLIB_svd_small_getHandleSize(&kerInitArgs);
   ...
   for (tpi = 0; tpi < test_cases; tpi++) {
      ...
      for (repCount = 0; repCount < numReps; repCount++) {
         ...
         void *pU, *pV, *pDiag, *pSuperDiag, *pU1;
         //  void *pV1, *pScratch;
         ...
         /* DebugComments- This condition needs to be checked */
            ...
            memset(pU, 0, uSize);
            memset(pV, 0, vSize);
            memset(pDiag, 0, diagSize);
            memset(pSuperDiag, 0, superDiagSize);
         ...
         if (pA && pV && pU && pU1 && pDiag && pSuperDiag) {
            ...
            status_init           = DSPLIB_svd_small_init_checkParams(handle, &bufParamsA, &bufParamsU, &bufParamsV,
                                                                      &bufParamsDiag, &bufParamsSuperDiag, &kerInitArgs);
            if (status_init == DSPLIB_SUCCESS) {
               TI_profile_start(TI_PROFILE_KERNEL_INIT);
               DSPLIB_asm(" MARK 0");
               status_init = DSPLIB_svd_small_init(handle, &bufParamsA, &bufParamsU, &bufParamsV, &bufParamsDiag,
                                                   &bufParamsSuperDiag, &kerInitArgs);
               DSPLIB_asm(" MARK 1");
               TI_profile_stop();
               if (status_init == DSPLIB_SUCCESS) {
                  status_opt = DSPLIB_svd_small_exec_checkParams(handle, pA, pU, pV, pDiag, pSuperDiag, pU1);
               }
               else {
            ...
            if (status_opt == DSPLIB_SUCCESS) {
               ...
               DSPLIB_asm(" MARK 2");
               status_opt = DSPLIB_svd_small_exec(handle, pA, pU, pV, pDiag, pSuperDiag, pU1);
               DSPLIB_asm(" MARK 3");
               TI_profile_stop();
            ...
            if (status_init == DSPLIB_SUCCESS) {
               ...
               if (status_init == DSPLIB_SUCCESS) {
                  status_nat = DSPLIB_svd_small_exec_checkParams(handle, pA, pUCn, pVCn, pDiagCn, pSuperDiagCn, pU1Cn);
               }
               else {
            ...
            if (status_nat == DSPLIB_SUCCESS) {
               ...
               DSPLIB_asm(" MARK 10");
               status_nat = DSPLIB_svd_small_exec(handle, pA, pUCn, pVCn, pDiagCn, pSuperDiagCn, pU1Cn);
               DSPLIB_asm(" MARK 11");
               TI_profile_stop();
            }
            /* ********************************************************************************** */
...
#endif
   {
      return DSPLIB_svd_small_d(&pProfile[0], 0);
   }
}
...
int coverage_test_main()
{
   ...
   status_init = DSPLIB_svd_small_init_checkParams(NULL, &bufParamsA, &bufParamsU, &bufParamsV, &bufParamsDiag,
                                                   &bufParamsSuperdiag, &kerInitArgs);
   ...
   bufParamsA.dim_x = 6;
   bufParamsA.dim_y = 6;
   status_init      = DSPLIB_svd_small_init_checkParams(handle, &bufParamsA, &bufParamsU, &bufParamsV, &bufParamsDiag,
                                                        &bufParamsSuperdiag, &kerInitArgs);
   currentTestFail  = (status_init != DSPLIB_ERR_INVALID_TYPE);
   fail             = ((fail == 1) || currentTestFail == 1) ? 1 : 0;

   bufParamsA.data_type         = DSPLIB_FLOAT64;
   bufParamsU.data_type         = DSPLIB_INT64;
   bufParamsV.data_type         = DSPLIB_FLOAT64;
   bufParamsDiag.data_type      = DSPLIB_FLOAT64;
   bufParamsSuperdiag.data_type = DSPLIB_FLOAT64;
   status_init     = DSPLIB_svd_small_init_checkParams(handle, &bufParamsA, &bufParamsU, &bufParamsV, &bufParamsDiag,
   ...
   bufParamsA.data_type         = DSPLIB_FLOAT64;
   bufParamsU.data_type         = DSPLIB_FLOAT64;
   bufParamsV.data_type         = DSPLIB_INT64;
   bufParamsDiag.data_type      = DSPLIB_FLOAT64;
   bufParamsSuperdiag.data_type = DSPLIB_FLOAT64;
   status_init     = DSPLIB_svd_small_init_checkParams(handle, &bufParamsA, &bufParamsU, &bufParamsV, &bufParamsDiag,
                                                       &bufParamsSuperdiag, &kerInitArgs);
   ...
   bufParamsU.data_type         = DSPLIB_FLOAT64;
   bufParamsV.data_type         = DSPLIB_FLOAT64;
   bufParamsDiag.data_type      = DSPLIB_INT64;
   bufParamsSuperdiag.data_type = DSPLIB_FLOAT64;
   status_init     = DSPLIB_svd_small_init_checkParams(handle, &bufParamsA, &bufParamsU, &bufParamsV, &bufParamsDiag,
                                                       &bufParamsSuperdiag, &kerInitArgs);
   currentTestFail = (status_init != DSPLIB_ERR_INVALID_TYPE);
   ...
   bufParamsV.data_type         = DSPLIB_FLOAT64;
   bufParamsDiag.data_type      = DSPLIB_FLOAT64;
   bufParamsSuperdiag.data_type = DSPLIB_INT64;
   status_init     = DSPLIB_svd_small_init_checkParams(handle, &bufParamsA, &bufParamsU, &bufParamsV, &bufParamsDiag,
                                                       &bufParamsSuperdiag, &kerInitArgs);
   currentTestFail = (status_init != DSPLIB_ERR_INVALID_TYPE);
   fail            = ((fail == 1) || currentTestFail == 1) ? 1 : 0;
   ...
   /* CASE 1004 - NULL PTR CASE*/
   status_init     = DSPLIB_svd_small_exec_checkParams(handle, NULL, pU, pV, pDiag, pSuperDiag, pU1);
   currentTestFail = (status_init != DSPLIB_ERR_NULL_POINTER);
   fail            = ((fail == 1) || currentTestFail == 1) ? 1 : 0;

   status_init     = DSPLIB_svd_small_exec_checkParams(handle, pA, NULL, pV, pDiag, pSuperDiag, pU1);
   currentTestFail = (status_init != DSPLIB_ERR_NULL_POINTER);
   fail            = ((fail == 1) || currentTestFail == 1) ? 1 : 0;

   status_init     = DSPLIB_svd_small_exec_checkParams(handle, pA, pU, NULL, pDiag, pSuperDiag, pU1);
   currentTestFail = (status_init != DSPLIB_ERR_NULL_POINTER);
   fail            = ((fail == 1) || currentTestFail == 1) ? 1 : 0;

   status_init     = DSPLIB_svd_small_exec_checkParams(handle, pA, pU, pV, NULL, pSuperDiag, pU1);
   currentTestFail = (status_init != DSPLIB_ERR_NULL_POINTER);
   fail            = ((fail == 1) || currentTestFail == 1) ? 1 : 0;

   status_init     = DSPLIB_svd_small_exec_checkParams(handle, pA, pU, pV, pDiag, NULL, pU1);
   currentTestFail = (status_init != DSPLIB_ERR_NULL_POINTER);
   fail            = ((fail == 1) || currentTestFail == 1) ? 1 : 0;

   status_init     = DSPLIB_svd_small_exec_checkParams(handle, pA, pU, pV, pDiag, pSuperDiag, NULL);
   currentTestFail = (status_init != DSPLIB_ERR_NULL_POINTER);
   fail            = ((fail == 1) || currentTestFail == 1) ? 1 : 0;
   ...
   /* CASE 1006 - u final */
   bufParamsA.data_type = bufParamsDiag.data_type = bufParamsDiag.data_type = bufParamsU.data_type =
       bufParamsV.data_type                                                 = DSPLIB_FLOAT64;
   bufParamsA.dim_x                                                         = 7;
   bufParamsA.dim_y                                                         = 6;
   bufParamsA.stride_y                                                      = 6 * sizeof(double);

   bufParamsU.dim_x    = 7;
   bufParamsU.dim_y    = 7;
   bufParamsU.stride_y = 7 * sizeof(double);

   bufParamsV.dim_x    = 7;
   bufParamsV.dim_y    = 7;
   bufParamsV.stride_y = 7 * sizeof(double);
   ...
   memset(pA, 0, 4096);
   double *superDiagFill = (double *) pSuperDiag;
   ...
   kerInitArgs.enableReducedForm = 0;
   memset(pA, 0, 4096);
   superDiagFill    = (double *) pSuperDiag;
   superDiagFill[0] = 1.414;
   status_init      = DSPLIB_svd_small_init(handle, &bufParamsA, &bufParamsU, &bufParamsV, &bufParamsDiag,
                                            &bufParamsSuperdiag, &kerInitArgs);
   if (status_init == DSPLIB_SUCCESS) {
   ...
   bufParamsU.dim_x    = 6;
   bufParamsU.dim_y    = 6;
   bufParamsU.stride_y = 6 * sizeof(double);

   bufParamsV.dim_x    = 6;
   bufParamsV.dim_y    = 6;
   bufParamsV.stride_y = 6 * sizeof(double);
   ...
   /* For every call of DSPLIB_svd_small_getHandleSize() the execution history is pushed
      as this function is the anchor point for LDRA in .cpp kernel files.
      Therefore calling DSPLIB_svd_small_getHandleSize() to push the execution history
      at the end of coverage test cases. */
   int32_t handleSize_LDRA = DSPLIB_svd_small_getHandleSize(&kerInitArgs);
