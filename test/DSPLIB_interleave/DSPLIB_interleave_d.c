...
int DSPLIB_interleave_d(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   DSPLIB_DEBUGPRINTFN(0, "%s\n", "Entering DSPLIB_interleave_d");
   ...
   DSPLIB_interleave_testParams_t *prm;
   DSPLIB_interleave_testParams_t  currPrm;
   DSPLIB_interleave_getTestParams(&prm, &test_cases);

   DSPLIB_interleaveInitArgs kerInitArgs;

   int32_t             handleSize = DSPLIB_interleave_getHandleSize(&kerInitArgs);
   DSPLIB_kernelHandle handle     = malloc(handleSize);

   TI_profile_init("DSPLIB_interleave");
#if !defined(EVM_TEST)
   // file IO for Loki benchmarking
   FILE *fpOutputCSV = fopen("DSPLIB_interleave.csv", "w+");
   ...
   for (tpi = 0; tpi < test_cases; tpi++) {
      ...
      for (repCount = 0; repCount < numReps; repCount++) {
         ...
         if (pIn && pOut && pOutCn) {
            ...
            status_init = DSPLIB_interleave_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
            if (status_init == DSPLIB_SUCCESS) {
               TI_profile_start(TI_PROFILE_KERNEL_INIT);
               DSPLIB_asm(" MARK 0");
               status_init = DSPLIB_interleave_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
               DSPLIB_asm(" MARK 1");
               TI_profile_stop();
               status_opt = DSPLIB_interleave_exec_checkParams(handle, pIn, pOut);
            }
            else {
            ...
            if (status_opt == DSPLIB_SUCCESS) {
               ...
               DSPLIB_asm(" MARK 2");
               status_opt = DSPLIB_interleave_exec(handle, pIn, pOut);
               DSPLIB_asm(" MARK 3");
               TI_profile_stop();
            ...
            if (status_init == DSPLIB_SUCCESS) {
               ...
               status_opt = DSPLIB_interleave_exec_checkParams(handle, pIn, pOutCn);
            }
            else {
               status_opt = status_init;
            }
            ...
            if (status_opt == DSPLIB_SUCCESS) {
               ...
               DSPLIB_asm(" MARK 10");
               status_opt = DSPLIB_interleave_exec(handle, pIn, pOutCn);
               DSPLIB_asm(" MARK 11");
               TI_profile_stop();
            }
            ...
            // counters
            fprintf(fpOutputCSV, "interleave, %d, %d, %d, -, %d, 1, 1, %d, %d, %3.2f\n", testNum, currPrm.testPattern,
            ...
            DSPLIB_interleave_perfEst(handle, &archCycles, &estCycles);
   ...
   DSPLIB_DEBUGPRINTFN(0, "Exiting DSPLIB_interleave_d with return fail: %d\n", fail);
...
#endif
   {
      return DSPLIB_interleave_d(&pProfile[0], 0);
   }
}
...
int coverage_test_main()
{
   ...
   while (testNum <= 1006) {
      ...
      case 1000:
         ...
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_NATC;
         status_nat             = DSPLIB_interleave_init_checkParams(NULL, &bufParamsIn, &bufParamsOut, &kerInitArgs);
      ...
      case 1001:
         ...
         bufParamsOut.data_type = DSPLIB_INT32;
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_NATC;
         status_nat             = DSPLIB_interleave_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
      ...
      case 1002:
         ...
         break;
      case 1003:
         status_nat      = DSPLIB_interleave_exec_checkParams(handle, NULL, pOut);
         currentTestFail = (status_nat != DSPLIB_ERR_NULL_POINTER);
         break;
      case 1004:
         status_nat      = DSPLIB_interleave_exec_checkParams(handle, pIn, NULL);
         currentTestFail = (status_nat != DSPLIB_ERR_NULL_POINTER);
         break;
      case 1005:
         bufParamsIn.data_type  = DSPLIB_INT128;
   ...
   /* For every call of DSPLIB_interleave_getHandleSize() the execution history is pushed
      as this function is the anchor point for LDRA in .cpp kernel files.
      Therefore calling DSPLIB_interleave_getHandleSize() to push the execution history
      at the end of coverage test cases. */
   int32_t handleSize_LDRA = DSPLIB_interleave_getHandleSize(&kerInitArgs);
