...
#include "../../src/DSPLIB_blkCopy2D/DSPLIB_blkCopy2D.h"
...
int DSPLIB_blkCopy2D_d(uint32_t *pProfile, uint8_t LevelOfFeedback)
{
   ...
   blkCopy2D_testParams_t *prm;
   blkCopy2D_testParams_t  currPrm;
   blkCopy2D_getTestParams(&prm, &test_cases);

   DSPLIB_blkCopy2DInitArgs kerInitArgs;

   int32_t             handleSize = DSPLIB_blkCopy2D_getHandleSize(&kerInitArgs);
   DSPLIB_kernelHandle handle     = malloc(handleSize);

   TI_profile_init("DSPLIB_blkCopy2D");

   FILE *fpOutputCSV = fopen("DSPLIB_blkCopy2D.csv", "w+");
   fprintf(fpOutputCSV, "Kernel, Test ID, Test type, Bit Width, Parameters, Width, Height, Stride, HW "
   ...
   for (tpi = 0; tpi < test_cases; tpi++) {
      ...
      for (repCount = 0; repCount < numReps; repCount++) {
         ...
         bufParamsIn.stride_y  = currPrm.strideIn; // Use stride from test case

         bufParamsOut.data_type = currPrm.dataType;
         bufParamsOut.dim_x     = currPrm.width;
         bufParamsOut.dim_y     = currPrm.height;
         bufParamsOut.stride_y  = currPrm.strideOut; // Use stride from test case

         DSPLIB_DEBUGPRINTFN(0, "data_type: %d width: %d height: %d rowStride: %d\n", bufParamsIn.data_type,
                             kerInitArgs.width, kerInitArgs.height, bufParamsIn.stride_y);

         void *pIn = (void *) TI_memalign(DSPLIB_L2DATA_ALIGNMENT, bufParamsIn.dim_y * bufParamsIn.stride_y);
         ...
         if (currPrm.outputDataLocation == DSPLIB_TEST_OUTPUT_HEAP) {
            ...
            pOutCn = (void *) TI_memalign(DSPLIB_L2DATA_ALIGNMENT, bufParamsOut.dim_y * bufParamsOut.stride_y);
#else
         ...
         if (pIn && pOut && pOutCn) {
            ...
            if (currPrm.dataType == DSPLIB_FLOAT32 || currPrm.dataType == DSPLIB_FLOAT64) {
               for (i = 0; i < currPrm.height; i++) {
                  TI_fillBuffer_float(prm[tpi].testPattern, 0, (void *) ((char *) pIn + i * bufParamsIn.stride_y),
                                      prm[tpi].staticIn, currPrm.width, 1, 0, dataWidth, testPatternString);
               }
            }
            else {
               for (i = 0; i < currPrm.height; i++) {
                  TI_fillBuffer(prm[tpi].testPattern, (uint8_t) 255, (void *) ((char *) pIn + i * bufParamsIn.stride_y),
                                prm[tpi].staticIn, currPrm.width, 1, 0, dataWidth, testPatternString);
               }
            }

            kerInitArgs.funcStyle = DSPLIB_FUNCTION_OPTIMIZED;
            status_init = DSPLIB_blkCopy2D_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
            if (status_init == DSPLIB_SUCCESS) {
               TI_profile_start(TI_PROFILE_KERNEL_INIT);
               DSPLIB_asm(" MARK 0");
               kerInitArgs.funcStyle = DSPLIB_FUNCTION_OPTIMIZED;
               status_init           = DSPLIB_blkCopy2D_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
               DSPLIB_asm(" MARK 1");
               TI_profile_stop();
            }

            status_opt = DSPLIB_blkCopy2D_exec_checkParams(handle, pIn, pOut);

            if (status_opt == DSPLIB_SUCCESS) {
               TI_profile_start(TI_PROFILE_KERNEL_OPT);
               DSPLIB_asm(" MARK 2");
               status_opt = DSPLIB_blkCopy2D_exec(handle, pIn, pOut);
               DSPLIB_asm(" MARK 3");
               TI_profile_stop();
            }
            ...
            for (k = 0; k < bufParamsOut.dim_x * bufParamsOut.dim_y; k++) {
               outSum += *pOutTemp;
               pOutTemp++;
            }
            ...
            DSPLIB_asm(" MARK 8");
            status_nat = DSPLIB_blkCopy2D_exec(handle, pIn, pOutCn);
            DSPLIB_asm(" MARK 9");
            TI_profile_stop();
            ...
            if (currPrm.dataType == DSPLIB_FLOAT32 || currPrm.dataType == DSPLIB_FLOAT64) {
               status_nat_vs_opt =
                   TI_compare_mem_2D_float((void *) pOut, (void *) pOutCn, 0.001, (double) powf(2, -10),
                                           bufParamsOut.dim_x, bufParamsOut.dim_y, bufParamsOut.stride_y, dataWidth);
            }
            else {
               status_nat_vs_opt = TI_compare_mem_2D((void *) pOut, (void *) pOutCn, 0, 0, bufParamsOut.dim_x,
                                                     bufParamsOut.dim_y, bufParamsOut.stride_y, dataWidth);
            }
            ...
            if (currPrm.staticOut != NULL) {
               status_ref_vs_opt =
                   TI_compare_mem_2D((void *) currPrm.staticOut, (void *) pOut, 2, 65535, bufParamsOut.dim_x,
                                     bufParamsOut.dim_y, bufParamsOut.stride_y, dataWidth);
            ...
            sprintf(desc, "%s generated input | width = %d, height = %d, rowStride = %d, Data Type %d",
                    testPatternString, currPrm.width, currPrm.height, bufParamsIn.stride_y, currPrm.dataType);
            uint64_t hwCycles = 0;
            ...
            uint64_t num_se_fetches =
                ((currPrm.width * currPrm.height * dataWidth) + (__C7X_VEC_SIZE_BYTES__ - 1)) / __C7X_VEC_SIZE_BYTES__;
            ...
            fprintf(fpOutputCSV, "blkCopy2D, %d, %d, %d, -, %d, %d, %d, %d, %d, %3.2f\n", testNum, currPrm.testPattern,
                    dataWidth * 8, currPrm.width, currPrm.height, bufParamsIn.stride_y, (int) hwCycles,
                    !currentTestFail, per_vector_cycles);
            DSPLIB_DEBUGPRINTFN(0, "%s", "Current Test Complete\n");

            TI_profile_add_test(testNum++, currPrm.width, hwCycles, 0, currentTestFail, desc);
         }
         else {
...
int coverage_test_main()
{
   ...
   while (testNum <= 1005) {
      ...
      case 1000:
         ...
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_NATC;
         status_nat             = DSPLIB_blkCopy2D_init_checkParams(NULL, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_OPTIMIZED;
         status_opt             = DSPLIB_blkCopy2D_init_checkParams(NULL, &bufParamsIn, &bufParamsOut, &kerInitArgs);
      ...
      case 1001:
         ...
         bufParamsOut.data_type = DSPLIB_INT32;
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_NATC;
         status_nat             = DSPLIB_blkCopy2D_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_OPTIMIZED;
         status_opt             = DSPLIB_blkCopy2D_init_checkParams(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
      ...
      case 1002:
         ...
         break;
      case 1003:
         status_nat      = DSPLIB_blkCopy2D_exec_checkParams(handle, NULL, pOut);
         currentTestFail = (status_nat != DSPLIB_ERR_NULL_POINTER);
         break;
      case 1004:
         status_nat      = DSPLIB_blkCopy2D_exec_checkParams(handle, pIn, NULL);
         currentTestFail = (status_nat != DSPLIB_ERR_NULL_POINTER);
         break;
      case 1005:
         bufParamsIn.data_type  = DSPLIB_INT128;
         bufParamsOut.data_type = DSPLIB_INT32;
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_NATC;
         status_nat             = DSPLIB_blkCopy2D_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         kerInitArgs.funcStyle  = DSPLIB_FUNCTION_OPTIMIZED;
         status_opt             = DSPLIB_blkCopy2D_init(handle, &bufParamsIn, &bufParamsOut, &kerInitArgs);
         currentTestFail        = ((status_nat != DSPLIB_ERR_INVALID_TYPE) || (status_opt != DSPLIB_ERR_INVALID_TYPE));
         break;
      default:
         break;
   ...
   /* For every call of DSPLIB_blkCopy2D_getHandleSize() the execution history is pushed
      as this function is the anchor point for LDRA in .cpp kernel files.
      Therefore calling DSPLIB_blkCopy2D_getHandleSize() to push the execution history
      at the end of coverage test cases. */
   int32_t handleSize_LDRA = DSPLIB_blkCopy2D_getHandleSize(&kerInitArgs);
...
#endif
   {
      return DSPLIB_blkCopy2D_d(&pProfile[0], 0);
   }
}
