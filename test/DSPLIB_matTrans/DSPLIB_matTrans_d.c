   ...
   for (tpi = 0; tpi < test_cases; tpi++) {
      ...
      for (repCount = 0; repCount < numReps; repCount++) {
         ...
         if (pIn && pOut && pOutCn) {
               ...
               if (currPrm.staticOut != NULL) {
                  if (currPrm.dataType == DSPLIB_FLOAT32 || currPrm.dataType == DSPLIB_FLOAT64) {
                     ...
                     status_ref_vs_opt = TI_compare_mem_2DDualStride(
                         (void *) pOut, (void *) currPrm.staticOut, 2, 65535, bufParamsOut.dim_x, bufParamsOut.dim_y,
                         bufParamsOut.stride_y, bufParamsOut.dim_x * DSPLIB_sizeof(bufParamsOut.data_type),
                         DSPLIB_sizeof(bufParamsOut.data_type));
                  }
            ...
            DSPLIB_matTrans_perfEst(handle, &archCycles, &estCycles, currPrm.dataType, bufParamsIn.dim_x);
